...
榜单和右侧待评价区独立滚动。右侧搜索框（⌘F）不区分大小写地匹配名称和原名：待评价区只显示匹配项，榜单中匹配的卡片高亮、其余变暗，回车定位到第一个匹配，Esc 清空。顶部采用紧凑布局，不显示拖拽快捷入口，为梯度排行保留更多空间。拖到另一张卡片前可以精确调整顺序，也可以拖回右侧待评价区。靠近滚动区上下边缘会自动滚动；取消拖拽或切换任务会立即停止。点击卡片查看详情、编辑信息、直接选级或从榜单移除。梯度榜单中悬停或用方向键选中卡片后，按 1–6 评级、0 移回待评价、⌫ 移除。⌘ 点击逐个多选、⇧ 点击连续多选、⌘A 全选，之后可以一起拖动、按键评级，或用底部选择栏批量操作；右键卡片打开快捷菜单，Esc 取消选择。快捷评价使用数字键 1–6，S 或 → 跳过当前动画，← 回看并修改上一部，同时展示原名、首播日期和简介。⌘Z / ⇧⌘Z 撤销与重做任意榜单操作（评价、添加、移除、编辑、重命名、删除榜单等），提示条中也可以直接撤销。“无关心”计入已评价进度。

应用菜单提供常用快捷键：⌘N 新建榜单、⇧⌘A 添加动画、⌘O 导入、⇧⌘E 导出 JSON、⌘E 导出图片、⌘F 搜索、⌘1 / ⌘2 切换梯度榜单与快捷评价、⇧⌘L 显示名称、⌃⌘S 侧栏。顶部的菜单按钮可完全收起或展开左侧任务栏。榜单默认隐藏卡片下方名称，可通过“显示名称”开关恢复；悬停查看完整名称，缺失封面时保留名称占位。侧栏和名称偏好在本机保存，切换任务及重启后继续生效。
