...
动画梯度排行是一款在电脑上给动画「排座次」的小工具：建一份榜单，把看过的动画放进 **夯、顶级、人上上、NPC、拉完了、无关心** 六个等级，最后导出成一张图片分享出去。所有数据都保存在你自己的电脑上，不需要注册账号。
...
这是 macOS 对未签名软件的正常提醒，只需要处理一次：

1. 在「应用程序」文件夹里 **右键点击** 动画梯度排行，选择 **打开**，再在弹窗里点 **打开**。
2. 如果弹窗里没有「打开」按钮（较新的 macOS），请打开 **系统设置 → 隐私与安全性**，滚动到底部，在提到动画梯度排行的地方点 **仍要打开**。
